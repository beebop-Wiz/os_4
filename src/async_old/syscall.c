...
void do_syscall(regs_t r) {
  ...
  case SYS_WAIT:
    if(SYS_A1 == (unsigned int) SUS_SCHED) {
      switch_ctx(r);
      break;
    } else if(SYS_A1 == (unsigned int) SUS_WAIT) {
      unsigned short pid = SYS_A3 & 0xFFFF;
      unsigned short flags = SYS_A3 >> 16;
      ptab[cur_ctx]->waitpid = pid;
      ptab[cur_ctx]->waitflags = flags;
    } else if(SYS_A1 == (unsigned int) SUS_STOPPED) {
      if(ptab[ptab[cur_ctx]->ppid]->waitflags & 0x4) {
	clear_wait(cur_ctx, 0x0300);
      }
    }
  ...
  case SYS_SET_HANDLER:
    ...
    break;
    /*  case SYS_WAIT_END:
    ptab[ptab[cur_ctx]->ppid]->waitpid = 0;
    ptab[ptab[cur_ctx]->ppid]->waitflags = 0;
    ptab[ptab[cur_ctx]->ppid]->suspend &= ~SUS_WAIT;
    ptab[ptab[cur_ctx]->ppid]->r->eax = SYS_A1;
    break;*/
  case SYS_KILL:
    if(SYS_A2 == 5) ptab[SYS_A1]->suspend &= ~SUS_STOPPED;
    else pqueue_callback(SYS_A1, ASYNC_TYPE_PSIG, 0, SYS_A2);
    break;
  case SYS_TCSETPGRP:
